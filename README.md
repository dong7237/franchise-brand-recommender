...
기존 초기 매칭에는 약 1~2분이 소요됐습니다. 가상 고객 1만 건으로 학습한 모델을 도입한 뒤 직접 사용·확인한 기준으로 거의 즉시 후보를 반환하도록 개선했습니다. 이 시간은 초기 매칭 단계의 관찰값이며, 이후 LLM 질의응답까지 포함한 전체 서비스 시간이나 정식 벤치마크 수치는 아닙니다.
...
반복되는 후보 선정은 학습 모델이 맡고, 정보공개서 조사와 설명은 LLM이 맡도록 역할을 나눴습니다. API 비용 절감액은 별도로 측정하지 않았습니다.
...
- **팀 결과:** 추천 후보를 정보공개서 질의응답, 3종 재무 시나리오 및 PDF 리포트에 연결한 웹 MVP를 구현·배포했습니다. 전체 서비스를 개인 단독 구현으로 주장하지 않습니다.

**AI 활용:** API 수집·DB 구성은 요구사항을 바탕으로 Codex를 활용해 구현했고, 가상 고객 생성에는 GPT를 활용했습니다. 이 저장소는 코드의 수작업 작성량보다 문제 정의, 데이터 활용, 모델 선택과 서비스 연결 과정을 보여줍니다.
...
**모델 점수는 수익률이나 성공 확률이 아닙니다.** 또한 최종 4개가 모두 모델 점수 상위 4개인 구조도 아닙니다.

## 4. 검증 결과

아래는 **전체 가상 고객 1만 건 실험 중 분리한 평가 고객 2,000명**에 최종 추천 조건을 적용한 저장 결과입니다. 공개 샘플의 재학습 결과와는 다릅니다.
...
초기 매칭 시간은 약 1~2분에서 거의 즉시 처리하는 수준으로 개선됐습니다(직접 사용 시 관찰). 같은 입력·실행 환경의 반복 측정 로그는 보관된 자료에서 확인되지 않아 백분율이나 밀리초 단위 절감률은 제시하지 않습니다.
...
전체 실험용 1만 건 데이터 및 원래 학습 모델은 포함하지 않았습니다. 작은 샘플은 코드와 출력 형태를 확인하기 위한 것으로 성능 평가에 사용하지 않습니다.
...
원본 폴더에 없었던 API 수집 및 SQL 생성 코드를 새로 작성해 과거 구현물처럼 포함하지 않았습니다. 해당 코드가 정리되면 수집·적재 과정의 재현 범위를 확장할 수 있습니다.
...
가상 고객과 비공개 수치 보완 데이터를 사용한 공모전 MVP이며, 실제 투자 의사결정용 데이터로 배포하는 저장소는 아닙니다.
